...
class ImagePostFourController extends Controller
{
    public function ImagePostIndexFour()
    {
        return view('blogger.posts.image_templates.editor_template_four');
    }

    public function ImagePostStoreFour(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'bottom_image_3' => 'required|image|mimes:jpeg,png,jpg,svg|max:3072',
        ...
        $imagePost = new ImagePostTemplateFour();
        ...
        $bottom_image1 = time() . 'bi1.' . $request->bottom_image_1->extension();
        $bottom_image_path1 = 'upload/blogger_image_post/' . $bottom_image1;
        $bottom_image2 = time() . 'bi2.' . $request->bottom_image_2->extension();
        $bottom_image_path2 = 'upload/blogger_image_post/' . $bottom_image2;
        $bottom_image3 = time() . 'bi3.' . $request->bottom_image_3->extension();
        $bottom_image_path3 = 'upload/blogger_image_post/' . $bottom_image3;
        ...
        $imagePost->bottom_image3 = $bottom_image3;
        ...
        Image::make($request->file('bottom_image_3'))->save($bottom_image_path3, 80, 'jpg');
    ...
    public function ImagePostShowFour($slug)
    {
        $post['post'] = ImagePostTemplateFour::where('slug', $slug)->where('blogger_id', Auth::guard('blogger')->id())->first();
        ...
        $post['related_posts'] = ImagePostTemplateFour::where('id','!=',$post['post']->id)->inRandomOrder()->get();
    ...
    public function ImagePostEditFour($id)
    {
        $post['post'] = ImagePostTemplateFour::where('id', $id)->where('blogger_id', Auth::guard('blogger')->id())->first();
        if ($post['post'] == null)
        {
            return back();
        }
        ...
        return view('blogger.posts.image_templates_edit.editor_template_four_edit', compact('post'));
    }

    public function ImagePostUpdateFour(Request $request, $id)
    {
        $validator = Validator::make($request->all(), [
            'title' => 'required',
            'headline_1' => 'required',
        ...
        $imagePost = ImagePostTemplateFour::where('blogger_id', Auth::guard('blogger')->id())
            ->where('id', $id)->first();
        if ($request->post_status == 1)
        {
            $imagePost->post_status = 1;
        ...
        {
            $bottom_image_1 = time() . 'bi1.' . $request->bottom_image_1->extension();
        ...
        {
            $bottom_image_2 = time() . 'bi2.' . $request->bottom_image_2->extension();
        ...
        if ($request->has('bottom_image_3') AND $request->bottom_image_3 != null)
        {
            $bottom_image_3 = time() . 'bi3.' . $request->bottom_image_3->extension();
            $bottom_image_3_path = 'upload/blogger_image_post/' . $bottom_image_3;
            $imagePost->bottom_image_3 = $bottom_image_3;
            Image::make($request->file('bottom_image_3'))->save($bottom_image_3_path, 80, 'jpg');
        }
