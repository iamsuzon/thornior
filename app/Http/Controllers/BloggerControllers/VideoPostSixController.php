...
class VideoPostSixController extends Controller
{
    public function VideoPostIndexSix()
    {
        return view('blogger.posts.video_templates.editor_template_six');
    }

    public function VideoPostStoreSix(Request $request)
    {
        ...
        $imagePost = new VideoPostTemplateSix();
        ...
        $imagePost->outro_headline = $request->last_headline;
    ...
    public function VideoPostShowSix($slug)
    {
        $post['post'] = VideoPostTemplateSix::where('slug', $slug)->where('blogger_id', Auth::guard('blogger')->id())->first();
        ...
        $post['related_posts'] = VideoPostTemplateSix::where('id','!=',$post['post']->id)->inRandomOrder()->get();
...
//        }
    ...
    public function VideoPostEditSix($id)
    {
        $post['post'] = VideoPostTemplateSix::where('id', $id)->where('blogger_id', Auth::guard('blogger')->id())->first();
        if ($post['post'] == null) {
            return back();
        }
        $post['products'] = BloggerProduct::where('blogger_id', Auth::guard('blogger')->id())
    ...
    public function VideoPostUpdateSix(Request $request, $id)
    {
        $validator = Validator::make($request->all(), [
            'title' => 'required',
            'category' => 'required',
        ...
        $imagePost = VideoPostTemplateSix::where('blogger_id', Auth::guard('blogger')->id())
            ->where('id', $id)->first();
        if ($request->post_status == 1) {
            $imagePost->post_status = 1;
        }
